...
function MobileAccountMenu({ username, open, onToggle, onClose, isAdmin }) {
  ...
  return (
    <div className="relative">
      ...
      {open && <div id="mobile-account-menu" className="absolute right-0 top-[calc(100%+0.65rem)] z-50 w-64 overflow-hidden rounded-2xl border border-graylight/20 bg-white text-graydark shadow-[0_18px_42px_rgba(30,64,100,0.24)]">
...
function MobileBottomNav({ pathname, onMenuOpen }) {
  ...
  return (
    <nav aria-label="เมนูหลักบนมือถือ" className="lg:hidden fixed inset-x-0 bottom-0 z-40 grid min-h-[4.9rem] grid-cols-5 border-t border-graylight/25 bg-white/95 px-1 pb-[env(safe-area-inset-bottom)] pt-1.5 shadow-[0_-8px_24px_rgba(30,64,100,0.08)] backdrop-blur-xl">
