...
function SubjectRadarChart({ subjectAccuracy }) {
  ...
  const valuePoints = values.map((value, index) => {
    const point = polarPoint(index, (value / 100) * RADAR_RADIUS);
    return `${point.x},${point.y}`;
  }).join(' ');
  const hasData = values.some((value) => value > 0);

  return (
    <div className="w-full rounded-2xl bg-white p-4 text-graydark shadow-[0_16px_32px_rgba(18,31,62,0.2)] sm:p-5 lg:ml-auto lg:w-[24rem]">
      ...
      <svg viewBox="0 0 260 260" className="mx-auto block w-full max-w-[18rem]" role="img" aria-label="กราฟเรดาร์แสดงความแม่นยำทั้ง 6 วิชา">
        <title>ความแม่นยำรายวิชา</title>
        {[20, 40, 60, 80, 100].map((value) => (
          <polygon key={value} points={polygonPoints((value / 100) * RADAR_RADIUS)} fill="none" stroke="#dbe3ef" strokeWidth="1" />
        ))}
        {subjects.map((subject, index) => {
          const point = polarPoint(index, RADAR_RADIUS);
          const label = polarPoint(index, RADAR_RADIUS + 26);
          ...
          return <g key={subject.id}>
            ...
            <text x={label.x} y={label.y + 3} textAnchor={anchor} className="fill-[#43516b] text-[9px] font-medium">{RADAR_LABELS[subject.id]}</text>
          </g>;
        })}
        ...
        <polygon points={valuePoints} fill="rgba(0,180,216,0.18)" stroke="#00b4d8" strokeWidth="2.5" strokeLinejoin="round" />
...
export default function ProfileHero({ subjectAccuracy }) {
  ...
  return (
    <section className="mb-7 overflow-hidden rounded-2xl border border-navy/10 bg-white shadow-[0_18px_42px_rgba(43,45,66,0.14)]" aria-labelledby="profile-title">
      <div className="relative overflow-hidden bg-[radial-gradient(circle_at_95%_10%,rgba(0,180,216,0.28),transparent_28%),radial-gradient(circle_at_12%_100%,rgba(216,176,107,0.16),transparent_32%),linear-gradient(120deg,#203f63,#2b5279)] p-5 sm:p-7">
        ...
        <div className="relative flex flex-wrap items-center gap-6 lg:gap-10">
          <div className="flex min-w-0 flex-1 items-center gap-4">
            <div className="animate-float grid h-16 w-16 shrink-0 place-items-center rounded-2xl bg-white/20 text-2xl font-bold text-white ring-1 ring-white/25 shadow-[0_10px_22px_rgba(18,31,62,0.25)]">{initial}</div>
          ...
          <SubjectRadarChart subjectAccuracy={subjectAccuracy} />
        </div>
      </div>
