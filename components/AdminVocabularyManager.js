    ...
    <div>
      <header className="mb-7 flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Link href="/vocab" target="_blank" className="inline-flex items-center gap-2 btn-outline"><ExternalLink size={16} />ดูหน้าผู้เรียน</Link>
      ...
      {!migrationRequired && <>
        ...
        {loading ? <div className="h-80 skeleton" /> : decks.length === 0 ? <section className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center"><BookOpenCheck size={30} className="mx-auto text-accent-cyan" /><h2 className="mt-3 font-semibold text-navy">เริ่มต้นคลังคำศัพท์</h2><p className="mx-auto mt-1 max-w-xl text-sm text-graydark/60">นำเข้าบัตรคำตัวอย่างชุดเดิมเพื่อให้หน้าผู้เรียนและหลังบ้านเริ่มจากข้อมูลชุดเดียวกัน จากนั้นค่อยแก้ไขหรือเพิ่มเนื้อหาจริงได้ตามต้องการ</p><button type="button" disabled={seeding} onClick={seedStarterLibrary} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-navy px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60">{seeding ? <LoaderCircle size={16} className="animate-spin" /> : <Sparkles size={16} />}นำเข้าชุดเริ่มต้น</button></section> : <>
          ...
          <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
            <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold text-navy">{selectedDeck?.title || 'บัตรคำ'}</h2><p className="mt-1 text-xs text-graydark/55">ข้อมูลด้านล่างคือข้อมูลเดียวกับที่ผู้เรียนเห็นในหน้าแฟลชการ์ด</p></div><div className="flex flex-wrap gap-2"><button type="button" disabled={!selectedDeck} onClick={() => { setDeckDraft(toDraft(selectedDeck)); setCardDraft(null); }} className="inline-flex items-center gap-2 btn-outline disabled:opacity-50"><PencilLine size={16} />แก้ไขชุด</button><button type="button" disabled={!selectedDeck} onClick={() => { setCardDraft(emptyCard(selectedDeckId, selectedDeckIsOxford ? 'A1' : '')); setDeckDraft(null); }} className="inline-flex items-center gap-2 rounded-xl bg-navy px-3.5 py-2.5 text-sm font-semibold text-white disabled:opacity-50"><FilePlus2 size={16} />เพิ่มบัตรคำ</button></div></div>
