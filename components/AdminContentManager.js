...
import { confirmArchiveQuestion } from '@/lib/sweetAlert';

const EMPTY_TOPIC = { subjectId: '', name: '', description: '' };
const EMPTY_SET = {
  bank: 'practice',
  title: '',
  slug: '',
...
export default function AdminContentManager() {
  ...
  const setsForQuestion = useMemo(
    () => content.sets.filter((item) => item.bank === question.bank && item.subject_id === question.subjectId && item.status !== 'archived'),
    [content.sets, question.bank, question.subjectId],
  );
  const activeQuestions = content.questions.filter((item) => item.is_active).length;
  const practiceSets = content.sets.filter((item) => item.bank === 'practice' && item.status !== 'archived').length;
  const mockSets = content.sets.filter((item) => item.bank === 'mock' && item.status !== 'archived').length;
  ...
  const saveTopic = async (event) => {
    ...
    setError('');
    try {
      await request('POST', { type: 'topic', ...topic });
      setTopic(EMPTY_TOPIC);
      setNotice('เพิ่มหมวดย่อยเรียบร้อยแล้ว');
      await refresh();
    } catch (saveError) {
      setError(saveError.message || 'เพิ่มหมวดย่อยไม่สำเร็จ');
    } finally {
      setSaving(false);
    }
  };
  ...
  const saveSet = async (event) => {
    ...
    setError('');
    try {
      await request('POST', { type: 'set', ...examSet });
      setExamSet(EMPTY_SET);
      setNotice('สร้างชุดข้อสอบแล้ว ต่อไปเพิ่มคำถามเข้าสู่ชุดนี้ได้เลย');
      await refresh();
    } catch (saveError) {
      setError(saveError.message || 'สร้างชุดข้อสอบไม่สำเร็จ');
    } finally {
      setSaving(false);
    }
  };
  ...
  const saveQuestion = async (event) => {
    ...
    try {
      ...
      setNotice(isEditing ? 'แก้ไขข้อสอบเรียบร้อยแล้ว' : 'เพิ่มข้อสอบเข้าสู่คลังเรียบร้อยแล้ว');
      await refresh();
    } catch (saveError) {
  ...
  const importBulkQuestions = async () => {
    ...
    try {
      const result = await request('POST', { type: 'bulk-questions', bank: bulkBank, items });
  ...
  return (
    <div className="max-w-6xl">
      <header className="mb-7 flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="mb-1 text-sm font-semibold text-cyan-600">ADMIN · QUESTION BANK</p>
          <h1 className="text-2xl font-bold tracking-tight text-navy sm:text-3xl">จัดการคลังข้อสอบ</h1>
          <p className="mt-1 text-sm text-graydark/60">สร้างคำถาม แบบฝึกหัด และ Mock Exam จากคลังแยกกันอย่างเป็นระบบ</p>
        </div>
        <div className="inline-flex items-center gap-2 rounded-2xl border border-cyan-100 bg-cyan-50 px-4 py-3 text-sm font-medium text-cyan-800">
          <Sparkles size={18} /> จำนวนข้อของแต่ละชุดนับอัตโนมัติ
        </div>
      </header>
      ...
      <section className="mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard icon={Database} label="ข้อสอบที่เปิดใช้" value={activeQuestions} tone="cyan" />
        <StatCard icon={ClipboardList} label="ชุดแบบฝึกหัด" value={practiceSets} tone="navy" />
        <StatCard icon={BookOpenCheck} label="ชุด Mock Exam" value={mockSets} tone="violet" />
        <StatCard icon={Tag} label="หมวดย่อย" value={content.topics.length} tone="amber" />
      </section>
      ...
      <div className="mb-6 flex gap-2 overflow-x-auto border-b border-slate-200" role="tablist" aria-label="เมนูจัดการคลังข้อสอบ">
        ...
        <TabButton active={tab === 'topics'} onClick={() => setTab('topics')} icon={Tag} label="หมวดวิชา" />
        <TabButton active={tab === 'announcements'} onClick={() => setTab('announcements')} icon={Sparkles} label="ประกาศ" />
      </div>
      ...
      {!loading && tab === 'questions' && (
        <QuestionTab
          ...
          onCancel={() => setQuestion(EMPTY_QUESTION)}
          onEdit={editQuestion}
          onArchive={archiveQuestion}
          bulkJson={bulkJson}
          setBulkJson={setBulkJson}
          bulkBank={bulkBank}
          setBulkBank={setBulkBank}
...
function QuestionTab({
  question, setQuestion, subjects, topics, sets, questions, saving, onSubmit, onCancel, onEdit, onArchive,
  bulkJson, setBulkJson, bulkBank, setBulkBank, bulkImporting, bulkResult, onBulkImport,
}) {
  const isEditing = Boolean(question.id);
  const setField = (key, value) => setQuestion((current) => ({ ...current, [key]: value }));
  const changeBank = (bank) => setQuestion((current) => ({ ...current, bank, setId: '' }));
  ...
  return <div className="space-y-7">
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
        <div><h2 className="text-lg font-bold text-navy">{isEditing ? 'แก้ไขข้อสอบ' : 'เพิ่มข้อสอบใหม่'}</h2><p className="mt-1 text-sm text-graydark/55">คำถามจะอยู่ในคลัง {question.bank === 'mock' ? 'Mock Exam' : 'แบบฝึกหัด'} และเลือกเพิ่มเข้าชุดได้ทันที</p></div>
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <label className="text-sm font-medium text-graydark">คลังข้อสอบ
            <select value={question.bank} onChange={(event) => changeBank(event.target.value)} className={fieldClass}><option value="practice">แบบฝึกหัดรายวิชา</option><option value="mock">Mock Exam</option></select>
          </label>
        ...
        <label className="block text-sm font-medium text-graydark">โจทย์ข้อสอบ
          <textarea required value={question.stem} onChange={(event) => setField('stem', event.target.value)} rows={4} placeholder="พิมพ์โจทย์หรือสถานการณ์ของข้อสอบ" className={fieldClass} />
        </label>
    ...
    <section>
      <div className="mb-3 flex items-center justify-between gap-3"><div><h2 className="text-lg font-bold text-navy">ข้อสอบล่าสุด</h2><p className="text-sm text-graydark/55">แสดงสูงสุด 80 ข้อล่าสุด</p></div><span className="rounded-full bg-slate-100 px-3 py-1.5 text-sm font-semibold text-graydark">{questions.length} ข้อ</span></div>
      {questions.length === 0 ? <EmptyState icon={FilePlus2} title="ยังไม่มีข้อสอบในคลัง" description="เริ่มจากกรอกคำถามแรก แล้วเลือกว่าจะเก็บไว้ในคลังหรือเพิ่มเข้าชุดทันที" /> : <div className="grid gap-3">{questions.map((item) => <QuestionCard key={item.id} item={item} onEdit={() => onEdit(item)} onArchive={() => onArchive(item.id)} />)}</div>}
    </section>
  </div>;
}
...
function BulkImportPanel({ bulkJson, setBulkJson, bulkBank, setBulkBank, bulkImporting, bulkResult, onBulkImport }) {
  ...
  return (
    <section className="rounded-3xl border border-cyan-200 bg-cyan-50/40 p-5 shadow-sm sm:p-6">
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="flex items-center gap-2 text-lg font-bold text-navy"><FileJson size={19} className="text-cyan-700" /> นำเข้าข้อสอบเป็น JSON (หลายข้อพร้อมกัน)</h2>
          <p className="mt-1 text-sm text-graydark/55">วางเป็น array ของข้อสอบ นำเข้าได้สูงสุด 300 ข้อต่อครั้ง — ข้อที่ผิดรูปแบบจะถูกข้าม ส่วนข้อที่ถูกต้องจะถูกบันทึกตามปกติ</p>
        </div>
      ...
      {showExample && (
        <pre className="mb-4 max-h-64 overflow-auto rounded-xl border border-slate-200 bg-slate-900 p-4 text-xs leading-relaxed text-slate-100">{BULK_EXAMPLE}</pre>
      )}

      <div className="mb-3 flex flex-wrap items-center gap-3">
        <label className="text-sm font-medium text-graydark">คลังข้อสอบเริ่มต้น
          <select value={bulkBank} onChange={(event) => setBulkBank(event.target.value)} className={`${fieldClass} w-auto`}>
            <option value="mock">Mock Exam</option>
            <option value="practice">แบบฝึกหัดรายวิชา</option>
          </select>
        </label>
        <p className="text-xs text-graydark/50">ใช้ค่านี้เมื่อข้อสอบแต่ละข้อไม่ได้ระบุ &quot;bank&quot; ของตัวเองใน JSON</p>
      </div>
...
function SetTab({ examSet, setExamSet, subjects, topics, sets, saving, onSubmit, onCancel }) {
  ...
  return <div className="space-y-7">
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      <div className="mb-5"><h2 className="text-lg font-bold text-navy">สร้างชุดข้อสอบ</h2><p className="mt-1 text-sm text-graydark/55">ไม่ต้องระบุจำนวนข้อ ระบบนับจากคำถามที่เพิ่มเข้าในชุดจริง</p></div>
      <form onSubmit={onSubmit} className="space-y-4">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <label className="text-sm font-medium text-graydark">ประเภทชุด<select value={examSet.bank} onChange={(event) => setField('bank', event.target.value)} className={fieldClass}><option value="practice">แบบฝึกหัดรายวิชา</option><option value="mock">Mock Exam</option></select></label>
          <label className="text-sm font-medium text-graydark">วิชา<select value={examSet.subjectId} onChange={(event) => changeSubject(event.target.value)} className={fieldClass}><option value="">ทุกวิชา / ยังไม่ระบุ</option>{subjects.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
          <label className="text-sm font-medium text-graydark">หมวดย่อย<select value={examSet.topicId} onChange={(event) => setField('topicId', event.target.value)} disabled={!examSet.subjectId} className={fieldClass}><option value="">ไม่ระบุหมวดย่อย</option>{topics.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
          <label className="text-sm font-medium text-graydark">สถานะ<select value={examSet.status} onChange={(event) => setField('status', event.target.value)} className={fieldClass}><option value="draft">ฉบับร่าง</option><option value="published">เผยแพร่</option></select></label>
        </div>
        <div className="grid gap-3 lg:grid-cols-[1fr_0.8fr_0.4fr_0.45fr]">
          ...
          <label className="text-sm font-medium text-graydark">เวลา (นาที)<input type="number" min="1" max="600" value={examSet.durationMinutes} onChange={(event) => setField('durationMinutes', event.target.value)} placeholder="ไม่กำหนด" className={fieldClass} /></label>
        </div>
...
function StatCard({ icon: Icon, label, value, tone }) {
  ...
  return <article className={`rounded-2xl border border-slate-200 p-4 ${tones[tone]}`}><Icon size={19} /><p className="mt-3 text-2xl font-bold">{value}</p><p className="mt-1 text-sm opacity-80">{label}</p></article>;
}
