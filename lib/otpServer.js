...
// Keep the browser session persistent across refreshes and browser restarts.
// The server still validates the signed value (and session_id when available)
// on every protected request, so this does not turn the cookie into a bearer
// token that can be trusted by itself.
export const AUTH_SESSION_TTL_SECONDS = 365 * 24 * 60 * 60;
...
export function createSession(phone, sessionId) {
  ...
  const payload = Buffer.from(JSON.stringify({
    ...
    sessionId,
    issuedAt: now,
    expiresAt: now + AUTH_SESSION_TTL_SECONDS * 1000,
  })).toString('base64url');
  return `${payload}.${sign(payload)}`;
}
...
// Set both Max-Age and Expires. Max-Age is sufficient in modern browsers, but
// the explicit expiry keeps the login persistent in mobile webviews that only
// retain cookies with an Expires attribute.
export function persistentCookieOptions(maxAge) {
  const ttl = Math.max(0, Number(maxAge) || 0);
  return {
    ...secureCookieOptions,
    maxAge: ttl,
