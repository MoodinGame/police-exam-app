...
export function recordTopicAttempt({ topicId, subjectId, score, total, answers = null, durationSeconds = null }) {
  if (!topicId) return;
  const now = new Date();
...
export function resetProgress() {
  ...
  try {
    ...
    // ล้างของ Mock Exam กับข้อสอบที่พักค้างไว้ด้วย ไม่งั้นแบนเนอร์ "ทำต่อ" จะยังโผล่หลังล้างสถิติ
    window.localStorage.removeItem('policeExam:mockAttempts');
    for (const key of Object.keys(window.localStorage)) {
      if (key.startsWith('policeExam:session')) window.localStorage.removeItem(key);
    }
  } catch {
    // ไม่ต้องทำอะไร
  }
...
const RESET_MARKER_KEY = 'policeExam:progressResetAt';

/**
 * ล้างความก้าวหน้าในเครื่องเมื่อแอดมินสั่งล้างสถิติหลังจากข้อมูลในเครื่องถูกบันทึกไว้
 * เซิร์ฟเวอร์ลบ localStorage ของผู้ใช้เองไม่ได้ จึงส่งเวลาที่สั่งล้างมาให้เทียบแทน
 */
export function syncProgressReset(serverResetAt) {
  if (typeof window === 'undefined' || !serverResetAt) return false;
  try {
    const seen = window.localStorage.getItem(RESET_MARKER_KEY);
    if (seen === serverResetAt) return false;
    resetProgress();
    window.localStorage.setItem(RESET_MARKER_KEY, serverResetAt);
    // ครั้งแรกที่เครื่องนี้เห็น marker (ยังไม่เคยบันทึก) ถือว่าล้างแล้ว แต่ไม่ต้องรีเฟรชหน้าให้เสียจังหวะ
    return seen !== null;
  } catch {
    return false;
  }
