 ...
 */
export function resolveSubjects(catalogSubjects, catalogTopics, topicQuestionCounts) {
  if (!Array.isArray(catalogSubjects) || catalogSubjects.length === 0) return [];

  const countsBySubject = (Array.isArray(catalogTopics) ? catalogTopics : []).reduce((counts, topic) => {
    const topicKey = topic.legacy_id || topic.id;
    const rawCount = Number(topicQuestionCounts?.[topicKey]);
    const questionCount = Number.isFinite(rawCount) && rawCount > 0 ? rawCount : 0;
    counts[topic.subject_id] = (counts[topic.subject_id] || 0) + questionCount;
    return counts;
  }, {});
  ...
  return catalogSubjects.map((row) => ({
    ...
    // จำนวนข้อต้องยึดจากคลังจริง ไม่ใช้ค่าคงที่เก่าที่อาจไม่ตรงกับข้อมูลแอดมิน
    count: countsBySubject[row.id] || 0,
  }));
}
...
export function useCatalog() {
  ...
  const subjects = useMemo(
    () => resolveSubjects(data?.subjects, data?.topics, data?.topicQuestionCounts),
    [data],
  );
